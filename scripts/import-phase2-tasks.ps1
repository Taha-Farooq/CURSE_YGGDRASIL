...
foreach ($m in $matches) {
    ...
    $id = "TASK-PHASE2-" + $num.PadLeft(2,'0') + "-" + (($title.ToUpper() -replace "[^A-Z0-9]+","-").Trim("-"))
    if (-not $taskMap.ContainsKey($id)) {
        $taskMap[$id] = @{
            id = $id
            title = $title
            source = "PHASE2_IMPLEMENTATION_PACK.md"
            status = "todo"
            priority = "high"
            type = "phase2"
        }
...
Write-Output "Imported Phase 2 tasks. Added: $added"
